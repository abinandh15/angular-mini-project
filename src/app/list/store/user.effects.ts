...
import { loadUsers, loadUsersError, searchUser, searchUserError,  updateUsersSuccess } from "./user.actions";
...
export class UserEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap(({searchQuery}) => this.appService.getUsers().pipe(
        map(users => updateUsersSuccess({users:users.filter((user:User) => user.username.toLowerCase().includes(searchQuery.toLowerCase()))})),
